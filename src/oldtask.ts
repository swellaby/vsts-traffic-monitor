...
const scanYesterday = async (user: VstsUser): Promise<VstsUsageRecord[]> => {
    try {
        const usageRecords = await utilizationService.getUserActivityFromYesterday(user.cuid, vstsAccountName, pat);
...
const scanLast24Hours = async (user: VstsUser): Promise<VstsUsageRecord[]> => {
    try {
        const usageRecords = await utilizationService.getUserActivityOverLast24Hours(user.cuid, vstsAccountName, pat);
        scanRecords(usageRecords, user);
        return usageRecords;
    } catch (err) {
